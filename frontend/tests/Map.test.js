// 1) Force the “enabled” path
process.env.NEXT_PUBLIC_USE_MAPBOX = 'true'

// 2) Stub out mapbox-gl completely
jest.mock('mapbox-gl', () => {
  ...
  return {
    ...
    flyToMock,  // expose our mock for assertions
    onMock,
  }
})

// 3) Now import *after* steps 1 & 2
...
describe('<Map/> Reset View', () => {
  ...
  it('renders the Reset View button and resets map view on click', async () => {
    ...
    // our stub should have been wired up to listen to "load"
    expect(onMock).toHaveBeenCalledWith('load', expect.any(Function))

    // now the Reset View button actually appears
    ...
    // flyTo gets called with your initial constants
